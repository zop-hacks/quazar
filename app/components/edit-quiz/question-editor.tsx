...
export function QuestionEditor({ questionData, onUpdate, className }: QuestionEditorProps) {
  ...
  return (
    <Card className={cn("w-full h-[calc(100%-4rem)] pb-0.5", className)}>
      <CardHeader>
        ...
        <CardDescription>Update the question, answers, and settings</CardDescription>
      </CardHeader>
        ...
        <div className="space-y-4">
              ...
              <Tooltip>
                ...
                <TooltipContent>
                  <p className="max-w-xs text-sm">
                    Select the checkbox next to the correct answer(s). You can have multiple correct answers.
                  </p>
                </TooltipContent>
              </Tooltip>
            </TooltipProvider>
          ...
          <div className="space-y-3">
            {answers.map((answer, index) => (
              ...
              >
                <div className="flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-full bg-zinc-100 text-sm font-medium dark:bg-zinc-800">
                  {answerLetters[index]}
                </div>
                ...
                <div className="flex items-center pt-2">
                  <Checkbox
                    ...
                    onCheckedChange={(checked) => handleAnswerCorrectChange(index, checked === true)}
                    className="h-5 w-5 data-[state=checked]:bg-green-600 data-[state=checked]:text-white"
                  />
                  <Label htmlFor={`correct-${index}`} className="ml-2 text-sm font-medium">
                    Correct
                  </Label>
                </div>
                <Button
        ...
        <div className="space-y-4 rounded-lg border border-zinc-200 p-4 dark:border-zinc-800">
          ...
          <div className="px-2">
            ...
            <div className="flex items-center justify-between text-sm text-zinc-500">
              <span>5 seconds</span>
              <span className="font-medium text-zinc-900 dark:text-zinc-50">{duration} seconds</span>
              <span>60 seconds</span>
            </div>
          </div>
        </div>
      </CardContent>
      <CardFooter className="flex justify-between border-t border-zinc-200 px-6 pb-7 dark:border-zinc-800">
        <Link href="/teach/my-quizzes"><Button variant="outline">Cancel</Button></Link>
