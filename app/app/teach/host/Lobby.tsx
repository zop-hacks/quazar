...
type useStateBool = React.Dispatch<React.SetStateAction<boolean>>;
type useStateStr = React.Dispatch<React.SetStateAction<string>>;
...
export const Lobby = ({
  ...
  setConnecting: useStateBool;
  setGameState: useStateStr;
  roomId: string | null;
}) => {
  ...
  return (
    <div className="relative h-[calc(100vh-4rem)] w-screen overflow-hidden bg-gradient-to-b from-sky-50 to-white flex justify-center flex-col items-center">
      ...
      >
        ...
        <div className="text-xl text-zinc-600 text-center mt-2">
          {quizTitle}
        </div>
      </motion.div>
      ...
      >
        {roomId ? (
          <div className="flex gap-3">
            ...
            <Card className="bg-white/90 border-zinc-200 shadow-lg p-6 flex flex-col items-center">
              <div className="text-zinc-500 mb-2">Qr Code</div>

              <motion.div
                className="text-5xl md:text-6xl lg:text-7xl font-bold text-zinc-800 tracking-wider mb-4 flex items-center gap-4"
                whileHover={{ scale: 1.05 }}
              >
                <QRCodeSVG className="p-3.5 h-full w-36" value={gameUrl} />
              </motion.div>
            </Card>
          </div>
        ) : (
      ...
      {/* Start Game button - only shown when players have joined */}
      {playerList && roomId && (
        <motion.div
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
        ...
        >
          <Button
            ...
            variant="default"
            size="lg"
            className="text-2xl py-6 px-10 bg-gradient-to-r from-green-500 to-emerald-600 hover:from-green-600 hover:to-emerald-700 text-white border-none shadow-lg hover:shadow-xl transition-all duration-300"
            onClick={startGame}
          >
            Start Game
          </Button>
      ...
      {playerList && Object.keys(playerList).length > 0 && (
        <div className="relative z-10 w-full max-w-3xl px-4">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
            <AnimatePresence>
              {Object.entries(playerList).map(
                ([clientId, [username, score]], index) => (
                  <motion.div
                    key={clientId}
                    initial={{ opacity: 0, y: 20 }}
                    animate={{ opacity: 1, y: 0 }}
                  ...
                  >
                    ...
                    >
                      ...
                      <div className="text-zinc-800 font-medium text-center truncate w-full">
                        {username}
                      </div>
                      {score > 0 && (
                        <Badge
